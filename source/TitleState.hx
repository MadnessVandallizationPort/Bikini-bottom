...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var water:FlxSprite = new FlxSprite(-600, -900).loadGraphic(Paths.image('sponge/Spongebob_BG' , 'shared'));
	...
	override function beatHit()
	{
		...
		switch (curBeat)
		{
			...
			case 5:
				if (Main.watermarks)
					createCoolText(['The Cool Engine', 'by']);
			case 7:
				if (Main.watermarks)
					FlxG.camera.flash(FlxColor.WHITE, 1);
	...
	function skipIntro():Void
	{
		...
		{
			remove(ngSpr);
			FlxTween.tween(gfDance, {y: 50}, {ease: FlxEase.expoInOut, startDelay: 0.2});

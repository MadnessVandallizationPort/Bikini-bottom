...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'mayonnaise': 
				dialogue = CoolUtil.coolTextFile(Paths.txt('mayonnaise/Mayonnaise'));
			case 'fun':
				dialogue = CoolUtil.coolTextFile(Paths.txt('fun/F.U.N.'));
			case 'e-minor':
				dialogue = CoolUtil.coolTextFile(Paths.txt('e-minor/E_Minor'));
		}
			...
			case 'water':
				{
						defaultCamZoom = 0.9;
						curStage = 'water';
						var bg:FlxSprite = new FlxSprite(-600, -900).loadGraphic(Paths.image('sponge/Spongebob_BG'));
						bg.antialiasing = true;
